  ...
  Layout = function () {_createClass(Layout, null, [{ key: 'CssClasses', get: function get() 
    ...
    function Layout(element) {_classCallCheck(this, Layout);
      ...
      event) {
        ...
        this.loadDoc('/views/components/' + fileName + '.html');} }, { key: 'toggleMinified', value: function toggleMinified() 
...

      {
        if (this.element_) {
          ...
          if (location.hash) {
            var _fileName = location.hash.split('#')[1];
            this.element_.querySelector('.' + Layout.CssClasses.TITLE).textContent = _fileName;
            this.loadDoc('/views/components/' + _fileName + '.html');}
...
(function () {
  ...
  Panel = function () {_createClass(Panel, null, [{ key: 'CssClasses', get: function get() 
    ...
    function Panel(element) {_classCallCheck(this, Panel);
      ...
      {
        this.reveal_.classList.add(Panel.CssClasses.IS_VISIBLE);} }, { key: 'onHideRevealPanel', value: function onHideRevealPanel() 

      {
        this.reveal_.classList.remove(Panel.CssClasses.IS_VISIBLE);} }, { key: 'init', value: function init() 
...
(function () {
  ...
  Select = function () {_createClass(Select, null, [{ key: 'CssClasses', get: function get() 
      {
        return { 
          SELECT: 'select', 
    ...
    function Select(element) {_classCallCheck(this, Select);
      this.element_ = element;
      this.init();}_createClass(Select, [{ key: 'selectElement', value: function selectElement() 

      {
        ...
        if (this.menuContainer_.classList.contains(Select.CssClasses.IS_VISIBLE)) {
          ...
          this.menuContainer_.setAttribute('aria-hidden', 'false');} else 
        {
          this.selectElement_.setAttribute('aria-expanded', 'false');
          this.selectElement_.setAttribute('aria-multiselectable', 'false');
          this.menuContainer_.setAttribute('aria-hidden', 'true');}} }, { key: 'removeScrollMask', value: function removeScrollMask() 
...

      {
        ...
        this.selectElement_.setAttribute('aria-expanded', 'false');
        this.selectElement_.setAttribute('aria-multiselectable', 'false');
        this.menuContainer_.setAttribute('aria-hidden', 'true');} }, { key: 'selectOption', value: function selectOption() 

      {
        var target = event.target;
        this.selectValue_.querySelector('span').textContent = target.querySelector('.' + Select.CssClasses.MENU_TEXT).textContent;

        for (var i = 0, len = this.menuOption_.length; i < len; i++) {
        ...
        this.removeScrollMask();} }, { key: 'init', value: function init() 

      {
        if (this.element_) {
          ...
          for (var i = 0, len = this.menuOption_.length; i < len; i++) {
            this.menuOption_[i].addEventListener('click', this.selectOptionHandler);}}} }]);return Select;}();
